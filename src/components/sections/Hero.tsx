  ...
  return (
    <section className="relative isolate overflow-hidden bg-gs-ink text-white">
      <Image
        ...
        priority
        sizes="100vw"
        className="object-cover object-[62%_center] opacity-[.72]"
      />
      <div
        aria-hidden
        className="absolute inset-0 bg-[linear-gradient(100deg,rgba(8,11,10,.96)_0%,rgba(8,11,10,.86)_38%,rgba(8,11,10,.35)_68%,rgba(8,11,10,.55)_100%)]"
      />
      <div
        aria-hidden
      ...
      <div className="gs-wrap relative z-10 pb-24 pt-16 sm:pb-32 sm:pt-24">
        ...
        <p className="mt-4 max-w-[52ch] text-[15.5px] leading-relaxed text-white/60">{t("lead")}</p>
        ...
        <dl className="mt-14 grid max-w-2xl grid-cols-2 gap-x-6 gap-y-7 border-t border-white/12 pt-8 sm:grid-cols-4">
          {stats.map((s) => (
            <div key={s.v}>
              ...
              <dd className="mt-1.5 text-[12.5px] leading-snug text-white/50">{s.v}</dd>
            </div>
          ))}
        </dl>
      </div>
